...
def normalize_date_to_iso(date_str: str | None) -> str | None:
    ...
    try:
        ...
        if re.match(r'^\d{4}-\d{2}-\d{2}$', date_str):
            datetime.strptime(date_str, '%Y-%m-%d') # 驗證有效性
            return date_str
        ...
        if re.match(r'^\d{7}$', date_str):
            ...
            if not (1 <= month <= 12 and 1 <= day <= 31):
                raise ValueError("月份或日期超出範圍")
            ad_year = roc_year + 1911
            # 格式化為 "YYYY-MM-DD"
            dt_str = f"{ad_year:04d}-{month:02d}-{day:02d}"
            datetime.strptime(dt_str, '%Y-%m-%d') # 再次驗證有效性
            return dt_str
    ...
    # 如果所有格式都匹配失敗
    print(f"警告: 未知的日期格式 '{date_str}'，無法正規化")
    return None